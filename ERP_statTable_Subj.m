...
function [SPSS_tab_Subj]=ERP_statTable_Subj(SubSelTW,inData,anzChan,Subj,Stim,G)
...
for g=1:G % groups
    ...
    for s=1:Subj % subjects
        SPSS_tab_Subj(Subj*(g-1)+s,1)=g;
        for st=1:Stim % stimuli
            time_window_tab(st,:)=SubSelTW(s).TWs(st,2:3,g); % selected_TW(st,2:3,g);
            mean_pow_sel_chan(:,st,s,g)=squeeze(mean(inData(anzChan,time_window_tab(st,1):time_window_tab(st,2),st,s,g),2));
            SPSS_tab_Subj(Subj*(g-1)+s,nCh*(st-1)+2:st*nCh+1)=[squeeze(mean_pow_sel_chan(:,st,s,g))'];
